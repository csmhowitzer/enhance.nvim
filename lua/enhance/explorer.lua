...
local function get_connection_tmp_dir(connection)
	...
	if not ok then
		vim.notify(string.format("Failed to create tmp directory: %s\nError: %s", conn_dir, err), vim.log.levels.ERROR)
		return nil
	end
...
local function fetch_tables(connection, force_refresh)
	...
	elseif db_type == "sqlserver" or db_type == "mssql" then
		...
		local cmd = {
			"sqlcmd",
			"-S",
			connection.server or connection.host,
			"-d",
			connection.database,
			"-h",
			"-1", -- Remove headers
...
local function delete_file_impl(filepath)
	...
	local do_delete = function()
		...
		if not ok then
			vim.notify(string.format("Failed to delete file: %s\nError: %s", filepath, result), vim.log.levels.ERROR)
			return
		end
